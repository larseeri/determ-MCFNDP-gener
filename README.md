...
https://github.com/larseeri/determ-MCFNDP-gener, or (ii) generate, download
locally to an appropriate folder and uncompress a .zip archive containing
the remote repository file tree.<br>
b- In a Linux terminal, go to the /program subfolder just created.<br>
...
'determ_gen' will be created inside the<br>/program folder.
...
line or in a configuration file. By default, the program will read option
values from file determ_config.par. (This default setting can be updated by
modifying line 808 of file main.cpp.) The default file can be overridden
with option +F:
...
with illustrative values and and should be edited appropriately if no 
...
this order:<br>
(i) hard-coded initialization values in lines 28-41 of data.cpp,<br>
...
taken from default configuration file whose name is specified in line 808
of main.cpp; at this point, the latter should exist,<br>
(iii) values specified in the command line.<br>
...
Two blank or comment rows (these are disregarded when reading)<br>
...
Three blank or comment rows (these are disregarded when reading)<br>
origNodeNumber destNodeNumber (a row identifies an arc from origin to destination)<br>
...
their resulting outputs (examined in Section 5 below).<br>
d- In a terminal, go to the /docker folder just created.

Then, instructions similar to those appearing in Section 2.2 can be run from
...
Section 2.2 by (i) adding the prefix 

	docker run --rm --mount type=bind,src=absolutePathToDockerFolder/inout,dst=/inout determ_gen_image

and by (ii) replacing the executable location ./determ_gen with /determ_gen
(i.e. removing the initial dot), since the executable determ_gen is located
...
respectively correspond to the following Docker instructions:
	...
	docker run --rm --mount type=bind,src=absolutePathToDockerFolder/inout,dst=/inout determ_gen_image \
	/determ_gen +v +F /inout/newParamfile.par
...
Similarly, a short output file name ./shortOutputFileName will be specified
in the Docker instruction or inside the configuration file with
-shortOutFileName /inout/shortOutputFileName.

The default configuration file is located at docker/inout/determ_config.par.
...
this order:<br>
(i) initialization values hard-coded in lines 28-41 of data.cpp,<br>
...
./inout/determ_config.par if no replacement configuration file is 
specified in the Docker instruction line with option key +F,<br>
(iii) values specified in the Docker instruction.<br>
Notice that location (i) is insufficiently transparent to the user and
should not be relied upon. Parameter values should be set through (ii) and
(iii).
...
through Docker instructions:<br>
...
this replaces the default file whose name is hard-coded in line 808 of
...
Remark:<br>
The output examples contained in the /examples subfolders will also be used
to illustrate the operation of the separate application generating stochastic
MCFND instances.
...
Docker instruction:
	...
	docker run --rm --mount type=bind,src=absolutePathToDockerFolder/inout,dst=/inout determ_gen_image \
	/determ_gen +v +F /inout/docker_determ_config_oneSrcOneSnk_10oct2025-18h10.par
...
Docker instruction:
	...
	docker run --rm --mount type=bind,src=absolutePathToDockerFolder/inout,dst=/inout determ_gen_image \
	/determ_gen +v +F /inout/docker_determ_config_eqSrcsEqsnks_10oct2025-18h10.par
